...
(function () {
    ...
    scrawl.prototype = {
        ...
        _addBoardListener:function (saveNum) {
            ...
            domUtils.on(canvas, ["touchstart", "touchmove", "touchend", "touchcancel"], function (e) {
                ...
                switch (e.type) {
                    case 'touchstart':
                        ...
                        isMouseMove = false;
                        me.isScrawl = true;
                        startX = e.touches[0].clientX - margin;//10为外边距总和
                        startY = e.touches[0].clientY - margin;
                        context.beginPath();
                        break;
                    case 'touchmove' :
                        if (!flag && button == 0) {
                        ...
                        var endX = e.touches[0].clientX - margin,
                            endY = e.touches[0].clientY - margin;
...
(function () {
    ...
    ScaleBoy.prototype = {
        init:function () {
            ...
            domUtils.on(scale, 'touchstart', function (e) {
                var target = e.target || e.srcElement;
                me.start = {x:e.touches[0].clientX, y:e.touches[0].clientY};
                if (target.className.indexOf('hand') != -1) {
                    me.dir = target.className.replace('hand', '');
                }
                domUtils.on(document.body, 'touchmove', me.scaleMousemove);
                e.stopPropagation ? e.stopPropagation() : e.cancelBubble = true;
            });
            domUtils.on(document.body, 'touchend', function (e) {
                if (me.start) {
                    domUtils.un(document.body, 'touchmove', me.scaleMousemove);
                    if (me.moved) {
...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        addMaskLayer('画板还是空的' + "&nbsp;&nbsp;&nbsp;<input type='button' value='" + '确定' + "'  onclick='removeMaskLayer()'/>");
    }
}
